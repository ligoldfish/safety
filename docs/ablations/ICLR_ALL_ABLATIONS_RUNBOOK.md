...
- 全计划 509 个单元：289 train、31 evaluate、186 analyze、3 manual；
- P0-07 是 6 数据集的 global × 3 方法，加 WJB/WGM 的 validation-selected × 3 方法，共 24 个公平预算单元；
- P0-07 之前另有 12 个验证集搜索候选（不计入正式 509 单元），由
...
1. 修复 split overlap 后运行 `core-train`、`wjb`，再完成 12 个 P0-07 validation-only
   搜索候选并收集账本，最后运行 24 个正式 `fairness` 单元；
